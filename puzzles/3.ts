console.log('---Advent of Code 2022 #3 ---');
...
const mockSackInput = 'RCZZQMQpzvRhswVg';
const lowerAlpha = 'abcdefghijklmnopqrstuvwxyz';
const upperALpha = lowerAlpha.toUpperCase();
function formatSacks(inputString: string): sackCompartment[]{
    let sacks = inputString.split('\n');
    let sacksWithCompartments = sacks.map((contents) => {
        let contentsArray = contents.split('')
        const firstHalf = contentsArray.slice(0, contentsArray.length / 2 );
        const secondHalf = contentsArray.slice(contentsArray.length / 2);
        return {
            firstHalf,
            secondHalf
        }
    });
    // console.log(sacksWithCompartments);
    return sacksWithCompartments;
}
...
function findSharedItems(sackObjs: sackCompartment[]): (string|undefined)[] {
    return sackObjs.map((sack) => {
        let sharedItem = undefined;
        for(let i = 0; i < sack.firstHalf.length; i++) {
            if(sharedItem){
                break;
            }
            sharedItem = sack.secondHalf.find((letter) => letter === sack.firstHalf[i]);
        }
        return sharedItem;
    });
}
let sharedItemList: (string)[] = findSharedItems(sacksWithCompartments) as unknown as string[];
console.log('sharedItemList', sharedItemList);
...
const listOfSharedItemValues = getSharedItemsValue(sharedItemList);
